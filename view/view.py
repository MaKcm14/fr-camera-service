...
class InfoDisplayView:
    ...
    def run(self):
        while True:
            ...
            self._flow.get_frame_png()
            # self._update_image(self._flow.get_frame_png())
            # self.root.update()
    ...
    #         # TODO: delete this. We'll use the server's status codes for chosing the status str.
    #         if t := int(time.time()) % 3:
    #             status = "Смотрите в камеру"
    #         elif t % 3 == 0:
    #             status = "Ваше изображение записано"
